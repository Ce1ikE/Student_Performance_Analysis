from chap_3_the_hunt_for_the_right_model.config import X_TRAIN, Y_TRAIN, X_TEST, Y_TEST
from main_config import (
    RANDOM_SEED,
    filename_results_ensemble_methods_pdf,
    filename_results_ensemble_methods_csv,
)

# from ai fundamentals =>
# bagging, boosting, stacking, voting, and blending
...
for i , model_res in enumerate(results.items()):
    ...
    if name in ensemble_models.keys():
...
results_df.to_csv(filename_results_ensemble_methods_csv)
# ----------------------------- Save Results ----------------------------- #
